...
function removeFirstOccurrences(str, value) {
  if (str.includes(value)) {
    const indOfVal = str.indexOf(value);
    const indOfvalLength = indOfVal + value.length;
    const firstPartOfString = str.slice(0, indOfVal);
    const secondPartOfString = str.slice(indOfvalLength);
    return firstPartOfString.concat('', secondPartOfString);
  }
  return str;
}
...
function removeLastOccurrences(str, value) {
  if (str.includes(value)) {
    const indOfVal = str.lastIndexOf(value);
    const indOfvalLength = indOfVal + value.length;
    const firstPartOfString = str.slice(0, indOfVal);
    const secondPartOfString = str.slice(indOfvalLength);
    return firstPartOfString.concat('', secondPartOfString);
...
function sumOfCodes(str) {
  const charArr = [];
  const notUndefinedStr = str ?? '';
  for (let i = 0; i < notUndefinedStr.length; i += 1) {
    charArr.push(str.charCodeAt(i));
  }
  return charArr.reduce((a, b) => {
    return a + b;
  }, 0);
}
...
function isPalindrome(str) {
  const clearStr = str.replace(/[^\w\\s]|/g, '');
...
function findLongestWord(sentence) {
  ...
  const arrSentenceWordLength = [];
  let indexBigWord;
  arr.forEach((element) => {
    arrSentenceWordLength.push(element.length);
    const maxWordLength = Math.max(...arrSentenceWordLength);
    indexBigWord = arrSentenceWordLength.indexOf(maxWordLength);
  });
  return arr[indexBigWord];
}
...
function encodeToRot13(str) {
  ...
  for (let j = 0; j < str.length; j += 1) {
    for (let i = 0; i < 13; i += 1) {
      ...
      if (str[j] === upperAlphabet[i]) {
        result[j] = upperAlphabet[i + 13];
      }
      if (str[j] === lowerAlphabet[i]) {
        result[j] = lowerAlphabet[i + 13];
      }
    }
  ...
  if (!lowerAlphabet.includes(str[str.length - 1])) {
    result[str.length] = str[str.length - 1];
  }
  return result.join('');
}
